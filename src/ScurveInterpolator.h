...
private:
	...
	static uint8_t profile_index(profile_segment *curve, double time)
	{
		uint8_t i;
		for (i = 1; i < 7; i++)
		{
...
public:
	...
	void setMoveData(float a, float j, float v, float vs, float ve)
	{
		max_acc = a;
		max_jer = j;
		max_vel = v;
	...
	bool update()
	{
		...
		{
			// v = profile_vel(segment, t);
			p = profile_pos(segment, t);
			// a = profile_acc(segment, t);
			// j = profile_jrk(segment, t);
		}
		...
		// v = profile_seg_vel(segment[6], segment[6].t);
		p = profile_seg_pos(segment[6], segment[6].t);
		// a = profile_seg_acc(segment[6], segment[6].t);
		// j = profile_seg_jrk(segment[6], segment[6].t);
		//p = p_target;
